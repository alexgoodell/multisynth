...
AudioSynthWavetable::instrument_data IntrumentList[] = {BasicFlute1, saw, Pizzicato, acid, cello};
...
void setup() {
  ...
  // display.display();
	...
	for (int i = 0; i < TOTAL_VOICES; ++i) {
		wavetable[i].setInstrument(BasicFlute1);
...
//	midi1.read();
  ...
  sgtl5000_1.volume(GlobalVolume); //from interface
...
void OnNoteOn(byte channel, byte note, byte velocity) {
	...
	switch (channel) {
	case 1:
		wavetable[wavetable_id].setInstrument(IntrumentList[selTest]);
		break;
	case 2:
		//wavetable[wavetable_id].setInstrument(saw);
		break;
	case 3:
